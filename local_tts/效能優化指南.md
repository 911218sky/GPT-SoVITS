...
詳細日常操作仍看：[操作指南](../../GPT-SoVITS-操作指南.md)（若在 workspace 根目錄） 與 [`README.md`](./README.md)。
...
| `speed_factor≠1.0` → 關 bucket，推理變慢（語速改留給 `tempo_audio.sh`） |
...
然後清音 →（可選）調速 → 合併，見操作指南。  
可選優化：TTS 產出編號 WAV 的同時，用另一個終端對已完成的檔跑 `process_audio.sh`（CPU 與 GPU 重疊）。
...
完整掃速表：`local_tts/output/_bench_sweep/results.json`。
微優化對照：`local_tts/output/_bench_micro/`。
